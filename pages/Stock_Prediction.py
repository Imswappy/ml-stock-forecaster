...
def get_data(
    ticker: str, years: int = 2, uploaded_csv_bytes: Optional[bytes] = None, show_diagnostics: bool = False
) -> pd.Series:
    """
    ...
    Attempts, in order:
        ...
        3) Twelve Data API (if TWELVEDATA_API_KEY provided in env)
        4) Alpha Vantage TIME_SERIES_DAILY_ADJUSTED (if ALPHAVANTAGE_API_KEY provided)
        5) yf.Ticker.history() fallback
    ...
    if uploaded_csv_bytes is not None:
        try:
            ...
            if not s.empty:
                if show_diagnostics:
                    print("get_data: using uploaded CSV")
                return s
        except Exception:
            if show_diagnostics:
                import traceback
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                if "Close" in df.columns:
                    ...
                    if not s.empty:
                        if show_diagnostics:
                            print(f"get_data: fetched {len(s)} rows from yfinance.download for {ticker}")
                        return s
        except Exception:
    ...
    td_key = os.environ.get("TWELVEDATA_API_KEY") or None
    # If running in Streamlit, user may have placed in st.secrets — we can't import streamlit here but the caller page can pass bytes if necessary.
    if td_key is None:
        # try environment only; callers can pass key via env or set secrets and copy to env in their platform
        td_key = None

    if td_key:
        try:
            # Twelve Data endpoint (daily)
            ...
            if r.status_code == 200:
                j = r.json()
                # response contains 'values' list of dicts with 'datetime','close','open','high','low','volume'
                ...
                if vals:
                    ...
                    if "close" in df.columns:
                        ...
                        if not s.empty:
                            if show_diagnostics:
                                print(f"get_data: fetched {len(s)} rows from TwelveData for {ticker}")
                            return s
            else:
                if show_diagnostics:
                    print(f"get_data: TwelveData HTTP {r.status_code}")
        except Exception:
            if show_diagnostics:
                import traceback
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if ts:
                    ...
                    # find close column name variants
                    col_map = {}
                    for c in df.columns:
                        lc = c.lower()
                        if "close" in lc and "adjusted" in lc:
                            col_map[c] = "Close"
                        elif "close" in lc:
                            col_map[c] = "Close"
                        elif "open" in lc:
                    ...
                    if "Close" in df.columns:
                        ...
                        if not s.empty:
                            if show_diagnostics:
                                print(f"get_data: fetched {len(s)} rows from AlphaVantage for {ticker}")
                            return s
            else:
                if show_diagnostics:
                    print(f"get_data: AlphaVantage HTTP {r.status_code}")
        except Exception:
            if show_diagnostics:
                import traceback
    ...
    # 4) yf.Ticker.history fallback (try again)
    if HAVE_YF:
        try:
            t = yf.Ticker(ticker)
            ...
            if df2 is not None and not df2.empty:
                ...
                if "Close" in df2.columns:
                    ...
                    if not s.empty:
                        if show_diagnostics:
                            print(f"get_data: fetched {len(s)} rows from yf.Ticker.history for {ticker}")
                        return s
        except Exception:
            if show_diagnostics:
                import traceback
    ...
    # All attempts failed -> empty series
    return pd.Series(dtype=float)
...
def get_rolling_mean(close_series: pd.Series, window: Optional[int] = None) -> pd.Series:
    ...
    # if rolling produced constant due to small variance, return a robust fallback
    try:
        if rolled.dropna().empty or (rolled.max() == rolled.min()):
            # fallback EWMA
            ewma_span = min(5, max(2, n // 10))
            ewma = close_series.ewm(span=ewma_span).mean().dropna()
            if not ewma.empty and ewma.max() != ewma.min():
                return ewma
...
def get_forecast(
    scaled_data: np.ndarray, d: int, steps: int = 30, last_date: Optional[pd.Timestamp] = None
) -> pd.DataFrame:
    """
