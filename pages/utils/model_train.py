...
def fit_model(data, differencing_order):
    # data should be a Series or DataFrame with single column
...
def get_forecast(original_price, differencing_order):
    # original_price passed here should match what fit_model expects (Series/array)
...
def inverse_scaling(scaler, scaled_data):
    """
    scaler: a fitted sklearn scaler
    scaled_data: 1D array-like or pandas Series of scaled values
    returns: 1D numpy array of inverse-transformed values
    """
    arr = np.array(scaled_data).reshape(-1, 1)
    close_price = scaler.inverse_transform(arr)
    # return flattened 1-D array
